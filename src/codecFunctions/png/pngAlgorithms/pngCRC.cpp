...
unsigned long update_crc(unsigned long crc,std::unique_ptr<unsigned char[]> & buf,
                         int len)
{
    unsigned long c = crc;
    int n;
...
/* Return the CRC of the bytes chunkName + rawData[0..len-1]. */
unsigned long crc(std::unique_ptr<unsigned char[]> & chunkName,std::unique_ptr<unsigned char[]> & rawData, int len)
{
    auto crc = update_crc(0xffffffffL, chunkName, 4);
    return update_crc(crc, rawData, len) ^ 0xffffffffL;
}